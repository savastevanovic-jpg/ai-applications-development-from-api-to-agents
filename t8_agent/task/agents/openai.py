...
class AnthropicBasedAgent(BaseAgent):
    ...
    def get_response(self, messages: list[Message], print_request: bool = True) -> Message:
        ...
        if response.status_code == 200:
            ...
            ai_response = Message(role=Role.ASSISTANT, content=text_blocks, tool_calls=tool_use_blocks)
    ...
    def _process_tool_calls(self, tool_calls: list[dict[str, Any]]) -> list[Message]:
        ...
        for tool_call in tool_calls:
            ...
            tool_message = Message(role=Role.USER, name=function_name, tool_call_id=tool_call_id, content=result)
